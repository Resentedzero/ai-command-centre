...
| **Registry** (6) | What are agents allowed to do? | Armory or vault: definitions and grants, with autonomy state per grant | 20–30% | Grants inside `AgentDetail` only | **Gated.** No write routes. Autonomy changes are safety-critical (spec §9.4). Pixel frame: "Registry — pixel armory (gated: read-only)" on the Artifacts/gated page. It is reached from the Agents tab (no top-bar slot), with one key per grant in the 4× room and a read-only line instead of controls. |
| **Cost** (7) | What is being consumed, by unit? | Hub telemetry room: one gauge per resource unit per scope, never summed | 20–30% | Per-run and per-agent budgets only | **Gated.** No scope-level read model. Pixel frame: "Cost — pixel ledger (gated: per run and per agent only)" on the Artifacts/gated page. It is reached from the Workflows tab. It shows one card per `resourceUnit` counter (limit, reserved, consumed, and an empty gauge until loaded) and per-agent `budgetTotals`. It has no world view (a dense data screen, like Events), no summed totals and no day/goal/global totals. |
