...
| `AgentDetail` | `GET /agents/:id` | role, objective, `activeStop`, grants (capability, permissions, `autonomyState`, trust, revoked), runs with goal lineage and latest invocation `kind`/`status`, `budgetTotals` per unit, `recentEvents`, `outputs`, `contextLineage` (tiers, exclusions, token estimate vs max), `performance` rows (see below). |
| `WorkflowRunSummary[]` | `GET /workflow-runs` | status, goal title, definition name/version, timestamps |
| `WorkflowRunDetail` | `GET /workflow-runs/:id` | ordered `steps` (task definition, task instance status, run, invocations with failure reason, `policyDecision` — Policy's recorded decision and basis, tool Invocations only, null otherwise — and `budgetOutcome` (`authorized` / `downgraded` / `degraded` / `denied` / null, the Governor's recorded outcome), budget per unit), `stepsUnavailableReason` |
...
| `AgentDetail.performance` (`AgentPerformanceRow[]`) | `GET /agents/:id` | per task definition and model tier: `sampleCount`, `successRate`, `avgRetries`, `avgCost` per unit, and `eligible` / `eligibilityReason` / `minSamples` from the runtime's gate (N = 10). **A measurement, not a ranking:** never rank, badge "best" or recommend from it, and never compare `sampleCount` with N in the UI. An eligible row can steer the Model Router's tier choice, so never claim performance cannot influence routing. Always show `sampleCount` beside it. |
...
| Policy decision (`PolicyDecisionRecord`) | `ALLOW`, `REQUIRE_APPROVAL`, `DENY`; `basis`: `no_grant`, `permission_not_granted`, `binding_below_grant_trust_bar`, `autonomy_autonomous`, `autonomy_always_approve`, `autonomy_state_unrecognized`, `unverified_binding_requires_approval`, `conditional_human_gated_action`, `conditional_insufficient_evidence`, `conditional_performance_meets_allow_threshold`, `conditional_performance_below_allow_threshold`, `conditional_performance_below_deny_threshold`; `autonomy_conditional_rule_undecided` on evaluations recorded before 2026-09-15. Show as words (`policyToken`): an allowed CONDITIONAL action reads `allowed · conditional`, neutral. `DENY` is red; `REQUIRE_APPROVAL` is amber only while the Invocation or Approval waits on a human (`policyTone`). `conditionalRule` and `performanceEvidence` go in a tooltip as given (`policyEvidenceTitle`). Never compute a decision, threshold or eligibility; no autonomy bars, threshold controls, promotion controls, or autonomy-driven lighting or sprites. |
| Route (`InvocationDetail.route`, `RouteRecord`) | LLM only. `tierSource`: `default`, `escalation_floor`, `performance_preference`, `budget_downgrade` (null on older routes). `budgetFallback`: the Governor's one fallback (tier denied and why it had been chosen, tier tried, context factor, input ceiling, authorized, and on a refusal `refusal` / `attemptedOutcome`), or null. `performance`: the rows the Router consulted, as recorded then (tooltip via `routeTitle`; a refused route's model reads "priced, not called"). Show as compact text (`MID · floor`, `STRONG · preference`, `CHEAP · budget downgrade`), the fallback as a tooltip (`budgetFallbackTitle`); never a light, sprite or progress state, never a control. |
| Attempt lineage (`attempts[]`) | `retryCause`: `provider_outcome_unknown`, `output_validation_failed`; `retryOfRunId`; `minimumModelTier`. Plain words. |
| Budget outcome (`InvocationDetail.budgetOutcome`) | `authorized`, `downgraded` (one tier lower after a denial), `degraded` (same tier, 75% Context Budget), `denied`, or null (no reservation recorded). Tools are only `authorized` / `denied`. The runtime's word as given, never inferred from event order: `denied` is red; the other three are neutral. Never amber. |
| Execution stop scope | `global`, `agent_definition`, `capability_grant`, `goal`, `workflow_run`, `run` |
| Autonomy | `ALWAYS_APPROVE`, `CONDITIONAL` (a low-risk READ is decided by recorded performance; everything else requires approval), `AUTONOMOUS` |
...
| "System Healthy" | No health read model. Show the real SSE connection state (connecting / live / reconnecting / offline) from `subscribeToActivity`. **Closed 2026-09-15:** `subscribeToActivity(since, onEvent, onStatus)` now reports `connecting | live | reconnecting`, and the client adds `offline` from `navigator.onLine` (`web/components/live.tsx`). |
