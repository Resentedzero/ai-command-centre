...
| Route (`InvocationDetail.route`, `RouteRecord`) | LLM only. `tierSource`: `default`, `escalation_floor`, `performance_preference` (null on older routes). Show as compact text (`MID · floor`, `STRONG · preference`); never a light, sprite or progress state, never a control. |
