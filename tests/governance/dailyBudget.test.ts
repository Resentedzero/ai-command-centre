...
describe("inert by default", () => {
  it("ships with NO daily ceiling for any unit, and cannot be raised at runtime", () => {
    expect(DAILY_BUDGET_CEILINGS).toEqual({});
    expect(Object.isFrozen(DAILY_BUDGET_CEILINGS)).toBe(true);
  });

  it("creates no day counter and returns the unchanged single-counter id", async () => {
    await withRollback(async (tx) => {
      const runId = await seedRun(tx, "subscription_tokens", "1000");
      const result = await reserveBudget(tx, "run", runId, "llm", "subscription_tokens", 100);
  ...
  it("keys days by UTC calendar date", () => {
    expect(dayScopeRef(new Date("2031-03-10T23:59:59.999Z"))).toBe("2031-03-10");
    expect(dayScopeRef(new Date("2031-03-11T00:00:00.000Z"))).toBe("2031-03-11");
  });
});
...
describe("no production caller can override the ceilings", () => {
  it("only budget.ts itself references the dailyCeilings injection option", () => {
