 ...
 * Run budget ceilings (`./runBudgetPolicy.ts`).
 *
...
function retryCause(facts: RunFailureFacts): RetryCause | null {
  if (facts.halted || facts.invocationKind !== "llm") return null;
...
/** Reads the failed Run's facts from its row and its own events. */
