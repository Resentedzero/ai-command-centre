 ...
 * agent, as the projector defines them). Below N, or with N unset, it influences
 * nothing; it stays displayable either way.
 *
 * SHIPPED INERT — NO VALUE IS INVENTED. `MIN_PERFORMANCE_SAMPLES` is null, so every
 * group is ineligible until an operator chooses N (the same pattern as
 * `./dailyBudgetPolicy.ts`). Callers may pass a different N only through an explicit
 * option, which production never does.
 *
...
/** N. Null = no criterion configured: nothing is eligible. */
export const MIN_PERFORMANCE_SAMPLES: number | null = null;
