...
3. **World:** Artifacts: the vault at 2x with one chest per output the API returned (the open one selected with a cream bracket; none drawn while loading). Events: a thin library strip only; the log is the screen.
