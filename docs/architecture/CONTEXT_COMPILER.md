...
| **Model-written directive text is fenced too (R2 Stage 14).** | `directive` is the runtime's own voice and must never carry model output. Text a model wrote for the instruction layer — today the Manager's delegated `brief` and `completionCriteria` — goes in `untrustedDirective`, gets the same fence, tag and policy as an artifact, and the trusted directive only points at it. Without this, injected text could leave one agent's fence and re-enter another's instruction channel. | `tests/api/manager.test.ts` |
...
**Output budget.** `expectedOutputTokens` is not used by the Compiler. The Model Router uses it for the reservation estimate (`maxInputTokens + expectedOutputTokens`, priced per model) and to cap the input budget at the routed model's window (`min(maxInputTokens, contextWindowTokens − expectedOutputTokens)`, §5.17), which the Executor passes to the Compiler.
...
4. **Not built:** compression (§5.9), caching (§5.11), relevance scoring (§5.3), memory (§5.6), §5.17's cache-block granularity (the window half is built), tokenizer-accurate estimates (`estimateTokens` is `ceil(chars / 4)`).
5. **No separate system channel on the Claude CLI** — see the spec §5.15 implementation note.
6. **§5.16 usage measurement (built 2026-09-14, Phase 20 #8).** `measureArtifactReferences` matches each included artifact's id in the output (case-insensitive, never a model call), and `invocation_completed` records `artifactReferences`: included and referenced ids and their estimated tokens. Models are not yet asked to cite ids, so it mostly records zero until a citation convention is decided (`ROADMAP_STATUS.md` §6). Cache hit/miss *per layer* is not measurable: providers report one cache read per call and no layer carries its own breakpoint.
