...
3. **Stranded mid-request work.** A request commits in several transactions, so its process can die, or its advance can throw, between them. Examples: an Approval committed but not yet advanced, or a dispatch recorded but the Run not continued.
   ...
   - **Without a restart**, the only repair is pause then resume: approve answers "already resolved", and resume only accepts `paused`. An explicit advance route would close this.
...
6. **Live SSE delivery is per request.** The relay publishes after the whole driver returns, so a long request's events reach live subscribers only at its end. They are committed, and visible on replay, as each transaction commits. Two concurrent requests on the same Workflow Run can each relay the other's events; clients deduplicate by event cursor.
