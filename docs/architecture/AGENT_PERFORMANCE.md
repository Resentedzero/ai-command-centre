...
| Measure | Rule | Test |
|---|---|---|
| **Sample** | A Run bound to an Agent Definition version whose terminal event is `run_completed` or `run_failed`, except outcomes that are not the agent's: any Run with a `run_halted` (operator stop), and a failed Run whose last `invocation_failed` reason is a governance, budget, quota, reauthorization, Approval-expiry, changed-binding or crash reason (`NOT_AGENT_OUTCOMES` in the projector), or whose `errorCode` is a provider refusal that consumed nothing, a context that did not fit its budget, a database error or a pre-dispatch refusal (`NOT_AGENT_ERROR_CODES`, since 2026-09-15), or a Run whose step could not be built or executed (`execution_error`). Every other failure counts, including `approval_rejected` (a human judged the work) and any unlisted reason: an unrecognized reason lowers the rate, which can never widen autonomy. Unfinished and unbound Runs are excluded. **Autonomous loops (operator decision 2026-09-16):** a completed Run whose loop concluded `incomplete` because the Budget Governor's headroom check stopped it (`budget_headroom`, `LOOP_STOPS_NOT_AGENT`) is not a sample, exactly as `insufficient_budget` is not. | `tests/projections/agentPerformance.test.ts`; mutation-checked |
...
| **success_rate** | Successful samples / samples. A sample succeeds when its Run completed **and**, if it ran an autonomous loop, that loop concluded `complete` (`evidence_sufficient` or `agent_finished`). A loop that concluded `incomplete` at its iteration or active-time limit is a failed sample: the Run ended cleanly, but the agent did not finish its work within its bounds. Until 2026-09-16 any completed Run was a success, which credited limit-stopped work as successful and fed that credit to tier preference and Conditional Autonomy. | same; mutation-checked (R2 cases) |
