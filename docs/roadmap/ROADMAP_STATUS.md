...
| **V4** | **Partial.** Built: the Model Router's measured tier preference (§10.2, §10.5) behind the sample criterion, live since N = 10 (2026-09-15). Built 2026-09-15: retries and the escalation loop (§3d, §10.4) under the operator's retry policy. Built 2026-09-15: `CONDITIONAL` autonomy under the operator's decision pack (a low-risk READ is decided by its routed tier's performance: ≥ 0.80 allow, ≥ 0.60 approval, below deny; everything else approval). Not built: evaluation (§8.9, evidence-gated). | `TIER_PREFERENCE_CLOSURE.md`, `RETRY_POLICY_CLOSURE.md`, `V1_DECISION_PACK_CLOSURE.md` |
...
| Minimum sample criterion and measured tier preference | V2 criterion; V4 (§10.2, §10.5) | **Closed.** `CONDITIONAL` autonomy built later (V1 decision pack). | `development/TIER_PREFERENCE_CLOSURE.md` |
...
| V1 decision pack | V4 (§9.4); Phase 4 Governor; §10.7 | **Closed 2026-09-15.** Conditional Autonomy built; Budget Governor downgrade/degrade built (`authorized` / `downgraded` / `degraded` / `denied`); routing observability reconciled; V1 readiness review fixes (projector classification by `errorCode`, a real JSON Schema for the research step). Open before dogfooding: a live check of the research step's `--json-schema`. | `development/V1_DECISION_PACK_CLOSURE.md` |
