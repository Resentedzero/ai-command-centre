...
| Governance decision records | V1 conformance (§8.2, §9.3); V2/V4 observability | **In progress 2026-09-15.** Policy decision record built (basis per decision, exposed on the Workflow view, trace and Approvals); per-Invocation budget outcome (`authorized` / `denied`). `CONDITIONAL` rule and Governor downgrade/degrade stay decisions (§6). | `development/GOVERNANCE_RECORDS_CLOSURE.md` |
