...
- ~~**Goal status lifecycle**~~ **Decided and built 2026-09-15 (R-GOAL1):** derived from the Goal's Workflow Runs, recorded as `goal_completed` / `goal_failed` / `goal_transitioned`. **Open for confirmation:** the derivation rule (`active` while any Workflow Run is unfinished or none exists; then `completed` if every one completed, else `failed`) and the `goal_transitioned` event are CLI1's reading; the decision named no rule. Residual: DURABLE_EXECUTION §7 #20.
