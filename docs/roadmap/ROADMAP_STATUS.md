...
| **V4** | **Partial.** Built: the Model Router's measured tier preference (§10.2, §10.5) behind the sample criterion, live since N = 10 (2026-09-15). Not built: `CONDITIONAL` autonomy (its thresholds are decisions, §6; it behaves as `ALWAYS_APPROVE`), the confidence-based escalation loop (§10.4 is retry machinery; retry policy is a decision), evaluation (§8.9, evidence-gated). | `TIER_PREFERENCE_CLOSURE.md`, `policy.ts` |
