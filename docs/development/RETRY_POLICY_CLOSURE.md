...
- ~~**Total spend per Task.** Three attempts × the Run ceiling, with no Task Instance counter (D20) and no day ceiling (D3).~~ **Decided and built 2026-09-15:** every attempt of a Task Instance shares one counter (`usd` 1.00, `subscription_tokens` 50,000), under a local-day ceiling (`usd` 5.00, `subscription_tokens` 200,000). `BUDGET_CONTAINMENT_CLOSURE.md`.
