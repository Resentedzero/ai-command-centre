...
- The projector also excludes by `errorCode` (`NOT_AGENT_ERROR_CODES`), by the `execution_error:` prefix, and a Run failed with reason `execution_error`. A timeout or a validation failure still counts (a timeout is not provably infrastructure; §4).
