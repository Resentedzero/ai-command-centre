...
- **Every attempt of a Task shares one Task Instance counter.** The first Run and each retry Run the retry policy starts (D1: up to 3 Runs) reserve against the same row, so no attempt can reserve once the Task's total reaches 50,000 tokens or $1.00, however many attempts it takes. Each Run's own counter still applies. The ceiling binds at reservation time: reconcile records what the provider reported, in full, so one call that overruns its estimate can leave a counter above its limit (every later reservation is then refused). This is true of every counter and is not clamped, since clamping would hide real consumption; capping provider output is the open `expected_output_tokens` decision (ROADMAP §6).
...
- **A pending approval keeps its reservation (CLI2 QA finding L1, left as built).** A Run parked awaiting approval holds its reservation against the applicable Run, day and Task Instance counters until the reservation is settled or released, so while it waits that amount is unavailable to other Runs today and to later attempts of the same Task Instance.
- **Lock order** is fixed: day → run → task_instance, in reserve, reconcile and release. Missing day and Task Instance rows are inserted just before those locks (DURABLE_EXECUTION §6).
- **Reservations made before this code.** A reservation id is decoded, not recomputed, so a Run-only reservation recorded before the upgrade (a Run's `budget_envelope.pendingReservations`) settles against the Run counter only. Before restarting any API on this code, check that no Run has a non-empty `pendingReservations`, as well as no unfinished Workflow Run. The local database had none on 2026-09-15.
...
- **Goal status.** After a Workflow Run starts or finishes, its Goal's status is re-derived from all its Workflow Runs. **The rule is confirmed by the operator (R-GOAL1, 2026-09-15, after CLI2's QA):**
  ...
  A change writes `goals.status` and emits the corresponding event, `goal_completed` or `goal_failed`, keyed by the Goal and the Workflow Run that caused it, payload `{ from, to, workflowRunId }`. `goal_transitioned` is retained for a future non-terminal Goal status transition; nothing currently triggers one (`POST /goals` starts exactly one Workflow Run per new Goal). An unchanged status writes nothing. Pause and resume never change it. All Workflow Run status writes are in the interpreter, so the sweep, approval expiry and ordinary advances all pass through this. `goals` joined the structural test's status tables. Since `goals.status` now reads `completed` or `failed`, the Goals screen shows those values; no UI code changed.
- **Backfill of existing Goals (CLI2 QA finding M1).** Goals whose Workflow Runs finished before this was built still read `active`. `backfillGoalStatuses` (`workflow/interpreter.ts`, run once with `npm run db:backfill-goal-status`) re-derives every Goal with the same rule and records each stale one through the same code: `goals.status` updated and one `goal_completed` / `goal_failed` event, producer `goal-status-backfill`, dated when the backfill ran (history is not back-dated), payload `{ from, to, workflowRunId, backfill: true }` naming the Goal's latest Workflow Run. It never writes a Workflow Run, locks the Goal rows first, and is idempotent: a second run finds nothing stale and writes nothing. Run on the local database 2026-09-15: 1 of 2 Goals updated (`active` → `failed`, its one Workflow Run failed); a second run updated 0.
...
- `tests/workflow/interpreter.test.ts`: pause/resume events across repeated cycles; `deriveGoalStatus`; `goal_completed`, `goal_failed` and `goal_transitioned` through real advances; the backfill updates only stale Goals, leaves Workflow Runs unchanged, and writes nothing on a second pass.
...
Migration `0019` was applied to the local database on 2026-09-15 with `npm run db:migrate`, after a `pg_dump` taken as a restore point. It adds triggers only and changed no row. Then the Goal status backfill ran (above). Any other database needs both steps, in that order: `npm run db:migrate`, then `npm run db:backfill-goal-status`. The API's startup re-drive rules are unchanged (check for unfinished Workflow Runs before restarting it).
