...
- **Goal status.** After a Workflow Run starts or finishes, its Goal's status is re-derived from all its Workflow Runs:
  ...
  A change writes `goals.status` and emits `goal_completed`, `goal_failed` or `goal_transitioned` (for example a finished Goal made active by a new Workflow Run), keyed by the Goal and the Workflow Run that caused it, payload `{ from, to, workflowRunId }`. An unchanged status writes nothing. Pause and resume never change it. All Workflow Run status writes are in the interpreter, so the sweep, approval expiry and ordinary advances all pass through this. `goals` joined the structural test's status tables.
